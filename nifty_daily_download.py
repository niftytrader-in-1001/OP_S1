...
def get_NIFTY_historical_data(smart_api, weeks=4):
    try:
        ...
        logger.error(f"NIFTY historical error: {e}")

    return None


def get_NIFTY_ltp(smart_api):
    try:
        params = {
            "exchange": "NSE",
            "symboltoken": NIFTY_TOKEN,
...
def calculate_strike_range(smart_api, buffer=500):
    hist = get_NIFTY_historical_data(smart_api, WEEKS_FOR_RANGE)
    if not hist:
...
def main():
    ...
    if success_list:
        ...
        send_zip_to_telegram(
            zip_buf.read(),
            f"NIFTY_expiry_{expiry.strftime('%d%m%y')}_1min.zip"
        )
